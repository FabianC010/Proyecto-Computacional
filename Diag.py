...
epsilon = np.full(1000, 10)                       # Valores energéticos de la diagonal principal.
t_enr = np.full(999, 1)                           # Valores energéticos de las diagonales secundarias.
...
psi = np.zeros(1000)
psi[psi.size//2] = 1.0                           # Declaramos el vector del estado inicial.
...
times = np.linspace(0.0, 30.0, 100, dtype = float)  # Se crea el arreglo de tiempos.
h = times[1] - times[0]                             # Se calcula el paso temporal que hay entre cada uno de ellos.

#Se declaran los distintos estados
stateQuant0 = np.zeros((times.size, psi.size), dtype = complex)
